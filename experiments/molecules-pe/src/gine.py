    ...
    def forward(
        ...
        for i, layer in enumerate(self.layers):
            ...
            if sketch_features is not None:
                X_n = torch.cat([X_n, sketch_features], dim=-1)

            X_n = layer(
                X_n, edge_index, edge_attr, PE
...
class GINELayer(MessagePassing):
    ...
    def __init__(
        ...
        if bond_encoder:
            ...
            self.edge_features = (
                nn.Embedding(n_edge_types + 1, in_dims)
                if feature_type == "discrete"
                else nn.Linear(n_edge_types, in_dims)
            )
    ...
    def forward(
        ...
        """
        X_e = (
            self.edge_features(edge_attr) if edge_attr is not None else None
        )  # [E_sum, D_in]
        if PE is not None:
