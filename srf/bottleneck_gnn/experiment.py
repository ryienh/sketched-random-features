...
class Experiment:
    def __init__(self, args):
        ...
        # OUR STUFF GOES HERE
        # Preprocess FIXME: fix hardcodes if possible
        import sys
        import os
        ...
        # R_global = torch.randn(1000, 1000)
        K_ = 16
        transform = AddFeaturesTransform(
            D_out=32, # NO NOT CHANGE THIS
            k=K_,
        )
        print(f"Using k = {K_}")
        self.X_train = [
            transform(datum) for datum in tqdm.tqdm(self.X_train, ncols=100)
        ]
        self.X_test = [transform(datum) for datum in tqdm.tqdm(self.X_test, ncols=100)]
        ...
        print(
            f"Training examples: {len(self.X_train)}, test examples: {len(self.X_test)}"
        )
    ...
    def run(self):
        optimizer = torch.optim.Adam(self.model.parameters(), lr=0.001)  # 0.001
        scheduler = ReduceLROnPlateau(
            optimizer, mode="max", threshold_mode="abs", factor=0.5, patience=4 # had tried 2 before, default is 10
        )
        ...
        for epoch in range(1, (self.max_epochs // self.eval_every) + 1):
            ...
            if epochs_no_improve >= self.patience:
                ...
                break
        print(
            f"Best train acc: {best_train_acc}, epoch: {best_epoch * self.eval_every}"
        )
