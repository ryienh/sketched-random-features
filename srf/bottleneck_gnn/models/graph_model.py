...
class GraphModel(torch.nn.Module):
    def __init__(
        ...
        self.out_layer = nn.Linear(
            in_features=h_dim, out_features=out_dim + 1, bias=False
        )

    def forward(self, data):
        x, edge_index, batch, roots = (
            data.x,
            data.edge_index,
            data.batch,
            data.root_mask,
        ...
        for i in range(self.num_layers):
            ...
            if self.gnn_type == GNN_TYPE.GINRBF:
                new_x = torch.cat([new_x, data.rbf_feats], dim=-1)
            elif self.gnn_type == GNN_TYPE.GINLaplace:
                new_x = torch.cat([new_x, data.laplace_feats], dim=-1)
            elif self.gnn_type == GNN_TYPE.GINLinear:
                new_x = torch.cat([new_x, data.linear_feats], dim=-1)
            elif self.gnn_type == GNN_TYPE.GINLaplaceNoSketch:
                new_x = torch.cat([new_x, data.laplace_feats_no_sketch], dim=-1)
            else:
                new_x = new_x
